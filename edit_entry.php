...
//$entry_fields = array('evento', 'name', 'description', 'start_date', 'end_date', 'areas',
//    'rooms', 'confirmation_status', 'reserva_oficina', 'evento_clases', 'privacy_status');
$entry_fields = array('evento', 'name', 'description', 'areas',
    'rooms', 'confirmation_status', 'reserva_oficina', 'evento_clases', 'evento_reunion',
	'evento_defensa', 'privacy_status', 'start_date', 'end_date');
...
function genSlotSelector($area, $prefix, $first, $last, $time, $display_none=FALSE, $disabled=FALSE)
{
  ...
  else
  {
	  ...
	  for ($t = $first; $t <= $last; $t = $t + $resolution)
	  {
	    ...
	    $slot_string = utf8_strftime($format, $timestamp);
	    $html .= "<option value=\"$t\"";
...
function create_field_entry_start_date($disabled=FALSE)
{
  ...
  foreach ($areas as $a)
  {
    ...
    if ($show_all_day && $ajax)
      $show_all_day = False;
                        
    if ($a['id'] == $area_id && $a['enable_periods']){
    	echo "<input type=\"hidden\" id=\"all_day{$a['id']}\" name=\"all_day\" value=\"yes\">\n";
    }
    else
    {
...
function create_field_entry_reserva_oficina($disabled=FALSE)
{
  //area_id: 5: Oficinas de trabajo
  ...
  // there is sonething there).   [\S]+.* is any non-whitespace character followed by any number
  // of other characters except a line terminator
  //generate_input("Nombre visitante:", 'nombre_visita', "", $disabled, $maxlength['entry.name'], 'type="text" required pattern="' . REGEX_TEXT_POS . '"');
  //generate_input($label_text, 'name', $name, $disabled, $maxlength['entry.name'], 'required pattern="[\S]+.*"');
  //echo "</div>\n";
  ...
  generate_input("País:", 'pais', $pais, $disabled, 50, 'type="text" pattern="' . REGEX_TEXT_POS . '"');
  echo "</div>\n";
...
function create_field_entry_evento_clases($disabled=FALSE)
{
  ...
  //         3: Salas publicas
  global $area_id;
  global $profesor, $curso;
  if($area_id == 5 or $area_id == 4)
    return;

  echo "<div id=\"div_evento_clase\">\n";
	  echo "<div id=\"div_profesor\" style=\"clear: both;\">\n";
	  generate_input("Profesor:", 'profesor', $profesor, $disabled, 80, 'type="text" pattern="' . REGEX_TEXT_POS . '"');
	  echo "</div>\n";

	  echo "<div id=\"div_curso\" style=\"clear: both;\">\n";
	  generate_input("Curso:", 'curso', $curso, $disabled, 80, 'type="text" pattern="' . REGEX_TEXT_POS . '"');
	  echo "</div>\n";
  echo "</div>\n";
}

function create_field_entry_evento_reunion($disabled=FALSE)
{
  //area_id: 5: Oficinas de trabajo
  //         4: Recursos computacionales
  //         3: Salas publicas
...
function create_field_entry_evento_defensa($disabled=FALSE)
{
  //area_id: 5: Oficinas de trabajo
  //         4: Recursos computacionales
  //         3: Salas publicas
...
function create_field_entry_evento($disabled=FALSE)
{
  //area_id: 5: Oficinas de trabajo
  //         4: Recursos computacionales
  //         3: Salas publicas
...
function create_field_entry_confirmation_status($disabled=FALSE)
{
  ...
  // TODO: sacar esto de aca, no se pq no me lo reconoce de otros lados.
...
// NOTE: fcifuent: agregado para pedir el formulario por ajax
...
if (isset($id))
{
  ...
  if ($row['area_id'] == $area_of_trabajo){
    $sql = "SELECT *
             FROM $tbl_entry, mrbs_entry_opt
             WHERE id=$id AND entry_id=$id
             LIMIT 1";
  }
  elseif ($row['area_id'] == $area_salas){
    $sql = "SELECT *
             FROM $tbl_entry, mrbs_entry_salas
             WHERE id=$id AND entry_id=$id
             LIMIT 1";
  }
  else
...
foreach ($edit_entry_field_order as $key)
{
  ...
  case 'reserva_oficina':
    create_field_entry_reserva_oficina();
    break;

  case 'evento_clases':
    create_field_entry_evento_clases();
    break;

  case 'evento_reunion':
    create_field_entry_evento_reunion();
    break;

  case 'evento_defensa':
    create_field_entry_evento_defensa();
    break;

  case 'evento':
	create_field_entry_evento();
	break;
